...
def signupPage(Request):
    ...
    if Request.method=="POST":
        ...
        if(p==cp):
            ...
            user=User.objects.create_user(username=b.username,email=b.email,password=p)
            if(user):
                user.save()
                b.save()
...
def addToCart(Request,id):
    # Request.session.flush() #This is used for clear session
        ...
        if(username):
            if(password==cpassword):
               ...
               return redirect('/login')
                ...
                messages.error(Request,"Password and Confirmed Doesn't  Matched !!!")
        else:
            messages.error(Request,"UnAuthorized User !!!")
